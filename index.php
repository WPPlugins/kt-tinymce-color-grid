...
if (!class_exists('kt_TinyMCE_Color_Grid')) {
    ...
    class kt_TinyMCE_Color_Grid {
        ...
        const WP_COLOR_PICKER_ALPHA = '~wp-color-picker-alpha(\.min)?\.js$~';
        ...
        public function enqueue_settings_scripts($hook) {
            if ($hook == 'settings_page_' . self::KEY) {
                ...
                if (!wp_script_is('name-that-color', 'registered')) {
                     ...
                     */
                    wp_register_script('name-that-color', 'http://chir.ag/projects/ntc/ntc.js', null, '1.0');
                }

                wp_enqueue_script(self::KEY, plugins_url('settings.js', __FILE__), array('jquery-ui-position', 'jquery-ui-sortable', 'name-that-color'), self::VERSION);
                wp_enqueue_style(self::KEY, plugins_url('settings.css', __FILE__), array('farbtastic'), self::VERSION);
                $picker = preg_replace(array('/\s*\n\s*/', '/"/'), array('', "'"), $this->picker);
                $picker = sprintf($picker, '#000000', 'rgba(0,0,0,1)', '100', 'Black', 'x', ' autofill');
        ...
        public function print_palette() {
            ...
            $alpha = get_option(self::ALPHA) && $this->wp_script_has(self::WP_COLOR_PICKER_ALPHA);
            $list = array();
            foreach ($palette as $set) {
                if ($alpha) {
                    $rgba = array_map('hexdec', str_split($set[0], 2));
                    $rgba[] = $set[1] / 100;
                    $list[] = '"rgba(' . esc_js(implode(',', $rgba)) . ')"';
                } else {
                    $list[] = '"#' . esc_js($set[0]) . '"';
                }
            }
            $list = array_pad($list, 6, $alpha ? '"rgba(0,0,0,0)"' : '"#FFFFFF"');
            $list = implode(',', $list);
            print '<script type="text/javascript">
  jQuery.wp.wpColorPicker.prototype.options.palettes = [' . $list . '];
</script>
';
        }
         ...
         * @return boolean
         */
        protected function wp_script_has($regex, $property = 'queue') {
            $wp_scripts = wp_scripts();
            foreach ($wp_scripts->$property as $obj) {
                $obj = is_object($obj) ? $obj : $wp_scripts->registered[$obj];
                if (preg_match($regex, $obj->src)) {
                    return true;
                }
            }
            return false;
...
.mce-grid tr:nth-child($rows) td {padding-bottom: 4px}
  ...
  <span class="sort hide-if-js">
    <button type="submit" name="kt_action" value="sort-%5$s-up" class="sort-up button" tabinde="3" title="%7$s">
      <i class="dashicons dashicons-arrow-up-alt2"></i>
      <span class="screen-reader-text">%7$s</span>
    </button>
    <button type="submit" name="kt_action" value="sort-%5$s-down" class="sort-down button" tabinde="3" title="%8$s">
  ...
  <span class="preview hide-if-js">
    <span class="rgb" style="background-color:%1$s"></span>
    <span class="rgba" style="background-color:%2$s"></span>
  </span>
  <span class="screen-reader-text">%10$s</span>
  <input class="hex" type="text" name="kt_colors[]" tabindex="3" value="%1$s" maxlength="7" placeholder="#RRGGBB" autocomplete="off" aria-label="%10$s" pattern="\s*#?([a-fA-F0-9]{3}|[a-fA-F0-9]{6})\s*" required="required" title="%15$s" />
  <span class="screen-reader-text">%11$s</span>
  <input class="alpha" type="number" min="0" max="100" step="1" name="kt_alphas[]" tabindex="3" value="%3$s" maxlength="3" autocomplete="off" aria-label="%11$s" title="%11$s" />
  ...
  <button type="submit" name="kt_action" value="remove-%5$s" tabindex="3" class="remove button">
    <i class="dashicons dashicons-trash"></i>
    <span class="screen-reader-text">%14$s</span>
  </button>
</div>', array(// hex    rgba    alpha   name    idx     autofill
                '%1$s', '%2$s', '%3$s', '%4$s', '%5$s', '%6$s',
                6 => esc_html__('Move up', 'kt-tinymce-color-grid'),
                7 => esc_html__('Move down', 'kt-tinymce-color-grid'),
                8 => esc_attr__('Visual Color Picker', 'kt-tinymce-color-grid'),
                ...
                14 => esc_attr__('3 or 4 hexadecimal numbers between 00 and FF', 'kt-tinymce-color-grid'),
            ));
        }
...
<p><a href="' . $support_url . '" target="_blank">' . esc_html__('Support Forums', 'kt-tinymce-color-grid') . '</a></p>');
        ...
        protected function save_settings() {
            ...
            foreach ($names as $i => $name) {
                $color = $this->sanitize_color($colors[$i], $alphas[$i]);
                if ($color) {
                    $color[2] = sanitize_text_field(stripslashes($name));
                    $palette[] = $color;
                }
            }
            $m = null;
            $l = count($palette);
            if ($action == 'add') {
                $palette[] = array('000000', '100', 'Black');
            ...
            update_option(self::ALPHA, $this->get('kt_alpha') ? '1' : false);
        ...
        protected function update_import(&$options) {
            ...
            while ($version != self::VERSION) {
                switch ($version) {
                    ...
                    case 181:
                    default:
                        foreach ($options[self::PALETTE] as &$color) {
                            array_splice($color, 1, 0, 100);
                        }
                        $version = self::VERSION;
                }
         ...
         * @since 1.4
         * @param string $rgb rgb string to be checked
         * @param string $alpha rgba [optional] string to be checked
         * @return array|boolean Returns an array of [RRGGBB, alpha] or false on failure
         */
        protected function sanitize_color($rgb, $alpha = 100) {
            $color = array('000000', 100);
            $rgb = strtoupper($rgb);
            $match = null;
            if (!preg_match('/([0-9A-F]{6}|[0-9A-F]{3})/', $rgb, $match)) {
                return false;
            }
            $color[0] = $match[1];
            if (strlen($color[0]) == 3) {
                $color[0] = preg_replace('/[0-9A-F]/', '\1\1', $color[0]);
            }
            if (preg_match('/100|[1-9][0-9]?|0/', $alpha, $match)) {
                $color[1] = intval($match[0]);
            }
            return $color;
        }
        ...
        protected function prepare_palette($rows = false, $cols = false) {
            ...
            if (get_option(self::VISUAL)) {
                ...
                if (count($palette)) {
                    ...
                    $padded = array_pad($palette[$last], $rows, array('FFFFFF', 'FF', ''));
                    $palette[$last] = $padded;
                }
            }
            return $palette;
        ...
        protected function add_palette(&$map, &$palette, $row) {
            ...
            for ($col = 0; $col < $cols; $col++) {
                ...
                $map[] = '"' . esc_js($color[0]) . '","' . esc_js($color[2]) . '"';
            }
        }
        ...
        /**
         * Apply a luma transformation on a RGB vector and return it as HEX string
         * @since 1.7
        ...
        <label for='$id' class='screen-reader-text'>$label</label>";
            ...
            foreach ($palette as $i => $set) {
                list($rgb, $alpha, $name) = $set;
                $r = hexdec(substr($rgb, 0, 2));
                $g = hexdec(substr($rgb, 2, 2));
                $b = hexdec(substr($rgb, 4, 2));
                $a = $alpha / 100;
                $picker .= vsprintf($this->picker, array(
                    esc_attr("#$rgb"),
                    esc_attr("rgba($r,$g,$b,$a)"),
                    esc_attr($alpha),
                    esc_attr($name),
                    $i,
                    $name ? '' : ' autofill'));
            ...
            $alpha_checked = $_alpha ? ' checked="checked"' : '';

            $alpha_channel = defined('ELEMENTOR_VERSION') ? "
            <input type='checkbox' id='kt_alpha' name='kt_alpha' value='1'$alpha_checked tabindex='9' />
            <label id='kt_alpha_option' for='kt_alpha'>" . esc_html__('Transparency', 'kt-tinymce-color-grid') . "</label>" : '';
      ...
      <div class='postbox-container'>$type_radios
        <div id='kt_color_grid_editor' class='postbox'>
          ...
          <div id='kt_palette_metabox' class='inside'>
            <input type='checkbox' id='kt_visual' name='kt_visual' value='1'$visual_checked tabindex='9' />
            <label id='kt_visual_option' for='kt_visual'>" . esc_html__('Add to Visual Editor', 'kt-tinymce-color-grid') . "</label>
            <input type='checkbox' id='kt_customizer' name='kt_customizer' value='1'$customizer_checked tabindex='9' />
            <label id='kt_customizer_option' for='kt_customizer'>" . esc_html__('Add to Theme Customizer', 'kt-tinymce-color-grid') . "</label>$alpha_channel
